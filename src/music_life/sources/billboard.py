...
def load_charts(con: duckdb.DuckDBPyConnection, data_dir: Path = VENDOR_DIR) -> dict[str, int]:
    ...
    for filename, (name, chart_type) in CHART_FILES.items():
        ...
        csv = f"read_csv(?, header = true, columns = {columns}, nullstr = ['NA', ''])"
        total = con.execute(f"SELECT count(*) FROM {csv}", [path.as_posix()]).fetchone()[0]
        ...
        con.execute(
            ...
            SELECT ?, chart_week, current_week, performer, title,
                   nullif(last_week, 0), peak_pos, wks_on_chart, ?
            FROM {csv}
        ...
        if inserted != total:
            print(f"warning: {name}: {total - inserted} rows share a week and position with another row and were skipped")
